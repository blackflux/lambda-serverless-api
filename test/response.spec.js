...
describe('Testing Response', () => {
  ...
  it('Testing auto field pruning', (done) => {
    api.wrap('GET test', [
      api.FieldsParam('fields', 'query', { allowedFields: ['foo'], autoPrune: true })
  ...
  it('Testing auto field pruning with path', (done) => {
    api.wrap('GET test', [
      api.FieldsParam('fields', 'query', { allowedFields: ['foo'], autoPrune: true, autoPrunePath: 'payload' })
    ], 10, (event, context, rb) => rb.warning('123')
      .then(() => api.JsonResponse({
        payload: {
          foo: 'bar',
  ...
  it('Testing pruneResponse false', (done) => {
    api.wrap('GET test', [
      api.FieldsParam('fields', 'query', { allowedFields: ['foo'], autoPrune: false })
    ], 10, (event, context, rb) => rb.warning('123')
      .then(() => api.JsonResponse({
        foo: 'bar',
        baz: 'quz'

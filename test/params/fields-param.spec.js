...
describe('Testing FieldsParam Parameter', () => {
  const queryParam = api.FieldsParam('param', 'query', { allowedFields: ['id', 'user.id', 'user.name'] });
  const jsonParam = api.FieldsParam('param', 'json', { allowedFields: () => 'id,user(id,name)' });
